...
extension SplashViewController: SplashPresenterOutputProtocol {
    func launchAnimation() {
        ...
        self.viewAnimator?.addAnimations {
            ...
            self.unblocledGR = Timer.scheduledTimer(timeInterval: 1.5, target: self, selector: #selector(self.automaticHandler), userInfo: nil, repeats: false)
        }
        self.viewAnimator?.startAnimation()
    }
}
